# ============================================================
#  ClipFlow Backend — Python Flask + yt-dlp
#  Railway veya Render'a deploy edilir
#  Destekler: YouTube, Instagram, TikTok, Twitter/X, Facebook
# ============================================================
...
def get_ydl_opts(job_id, quality="720", format_type="mp4", remove_audio=False):
    ...
    if remove_audio:
        opts["postprocessor_args"] = ["-an"]  # Ses kaldır
...
def get_info():
    """Video bilgisini getir (indirmeden önce önizleme)"""
...
def start_download():
    ...
    format_type  = data.get("format", "mp4")        # mp4, mp3, mov
...
def split_video():
    """
    Video bölme endpoint'i
    Gerçek implementasyonda FFmpeg kullanılır.
    Bu örnek iş akışını gösteriyor.
    """
    data = request.get_json()
    url      = data.get("url", "")
    duration = data.get("duration", 60)  # saniye cinsinden parça süresi
    platform = data.get("platform", "tiktok")

    # Platform bazlı max süre
    max_durations = {
        "tiktok":    60,
        "instagram": 90,
        "youtube":   60,
    ...
    max_dur = max_durations.get(platform, 60)
    ...
    # 1. yt-dlp ile videoyu indir
    # 2. FFmpeg ile parçalara böl: ffmpeg -i input.mp4 -segment_time 60 -f segment output%03d.mp4
    # 3. Parçaları ZIP'le ve gönder

    return jsonify({
        "job_id": job_id,
        "message": f"Video {max_dur} saniyelik parçalara bölünecek",
    })
